...
function idle(s::ServiceNode, ::Feed)
    if !isempty(s.input)
        ...
        debug[1] && now!(s.clk, fun((a,b,c)->@printf("%5.3f: %d working on job %d\n", a,b,c), tau(s.clk), s.id, s.job))
    end
end
busy(s::ServiceNode, ::Message) = nothing
function busy(s::ServiceNode, ::Finish)
    debug[1] && now!(s.clk, fun((a,b,c)->@printf("%5.3f: %d finished job %d\n", a,b,c), tau(s.clk), s.id, s.job))
    if !isfull(s.output)
        push!(s.output, s.job)
        send!((self(), s.cst), Feed())
        become(idle, s)
    else
...
- `coll<:AbstractArray{Int64,1}`: service node ident numbers, 
...
function line(s::ServiceNode, coll::T, buf::Int, ::Setup) where T<:AbstractArray{Int64,1}
    ...
    map(l) do i
        ...
        register!(i.clk.channels, i.act) # register them and
        send!(i.act, Setup())            # set them up
    end
    for i in 1:(length(coll)-1)           # create buffers and connect everything
...
Random.seed!(2020)          # set random number seed for reproducibility
...
sn.act = Actor(line, sn, [100,200,300], 5)
